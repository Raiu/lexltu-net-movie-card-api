...
{
    private static readonly Faker _faker = new("sv");
    ...
    private static List<Movie> GenerateMovies(List<Actor> actors, List<Director> directors, List<Genre> genres, int numberOf)
    {
        ...
        for (var i = 0; i < numberOf; i++)
        {
            var selectedActors = actors
                    .OrderBy(x => _rnd.Next())
                    .Take(_rnd.Next(1, Math.Min(actors.Count, 11)))
                    .ToList();

            var selectedGenres = genres
                    .OrderBy(x => _rnd.Next())
                    .Take(_rnd.Next(1, Math.Min(genres.Count, 3)))
                    .ToList();

            var movie = new Faker<Movie>("sv");
            movie.RuleFor(m => m.Title, f => f.Company.CatchPhrase());
            movie.RuleFor(m => m.Description, f => string.Join(" ", f.Lorem.Words(f.Random.Number(10, 30))));
            movie.RuleFor(m => m.ReleaseDate, f => f.Date.FutureDateOnly(5, DateOnly.FromDateTime(DateTime.Today)));
    ...
    private static List<Actor> GenerateActors(int numberOf) =>
        Enumerable.Range(1, numberOf)
            .Select(x => new Faker<Actor>("sv")
                .RuleFor(a => a.Name, f => f.Name.FullName())
                .RuleFor(a => a.DateOfBirth, f => DateOnly.FromDateTime(f.Date.Past(80, DateTime.Now.AddYears(-18))))
                .Generate()
            ).ToList();

    private static List<Director> GenerateDirectors(int numberOf) =>
        Enumerable.Range(1, numberOf)
            .Select(x => new Faker<Director>("sv")
                .RuleFor(d => d.Name, f => f.Person.FullName)
                .RuleFor(d => d.DateOfBirth, f => DateOnly.FromDateTime(f.Date.Past(80, DateTime.Now.AddYears(-40))))
                .RuleFor(d => d.ContactInformation, f =>
                    new Faker<ContactInformation>("sv")
                        .RuleFor(ci => ci.Email, _ => f.Person.Email)
                        .RuleFor(ci => ci.PhoneNumber, _ => (string)f.Person.Phone)
                        .Generate())
                .Generate()
            ).ToList();
